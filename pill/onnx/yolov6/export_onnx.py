...
if __name__ == '__main__':
    ...
    model = load_checkpoint(opt.weights, map_location=device, inplace=True, fuse=True)  # load FP32 model
    labels = model.names
    ...
    model.export = not opt.grid  # set Detect() layer grid export
    y = model(img)  # dry run
    if opt.include_nms:
        model.include_nms = True
        y = None
