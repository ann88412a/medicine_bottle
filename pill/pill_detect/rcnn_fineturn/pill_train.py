...
class PillDataset(object):
    ...
    def __getitem__(self, idx):
        ...
        iscrowd = torch.zeros((boxes.shape[0],), dtype=torch.int64) # 0 : each detect , 1 : group detect
...
def get_model_instance_pill(num_classes):
    # load an instance model pre-trained pre-trained on COCO
    model = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=True)

    # get number of input features for the classifier
    in_features = model.roi_heads.box_predictor.cls_score.in_features
    print('in featurs : ',in_features)
...
def main():
    ...
    # our dataset has two classes only - background and pill
    num_classes = 2
    ...
    indices = torch.randperm(len(dataset)).tolist() # torch.randperm (return randome shift index ex: [0,1,2] -> [1,0,2])
    dataset = torch.utils.data.Subset(dataset, indices[:-50]) # subset use index to spilt dataset
    ...
    model = get_model_instance_pill(num_classes)
