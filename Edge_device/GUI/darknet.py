...
if os.name == "posix":
    ...
    lib = CDLL(cwd + "/config_files/libdarknet.so", RTLD_GLOBAL)
elif os.name == "nt":
    # cwd = os.path.dirname(__file__)
    cwd = os.path.dirname(os.path.dirname(__file__))
    os.environ['PATH'] = cwd + ';' + os.environ['PATH']
    lib = CDLL("./config_files/darknet.dll", RTLD_GLOBAL)
else:
...
def np_image_to_c_IMAGE(input_frame):
    ...
    C_IMAGE_frame = make_image(w, h, c)
...
if __name__ == '__main__':
    names = ['Sennoside','Apresoline','Repaglinide','Cataflam']
    yolov4,classes,colors = load_network('/home/fritingo/Documents/yolo/darknet/pill/yolo_pill.cfg','/home/fritingo/Documents/yolo/darknet/pill/obj.data','/home/fritingo/Documents/yolo/darknet/pill/yolo_pill_final.weights')
    image = cv2.imread('/home/fritingo/Documents/pill_data/1/43.png')
    print(image.shape)
    img = np_image_to_c_IMAGE(image)

    # '/home/fritingo/Documents/yolo/darknet/pill/obj.name'
    # detection = detect_image(yolov4,names , img, thresh=.5, hier_thresh=.5, nms=.45)
   
    detection = detect_image(yolov4,names , img, thresh=.5, hier_thresh=.5, nms=.45)
    print(detection)
    cv2.imwrite('predict.png',draw_boxes(detection, image, class_colors(names)))
    # print_detections(detection)