...
class pill_yolo_2:
    def __init__(self):
        ...
        # init  
        # class name
        names = ['Concor', 'Isoptin 40mg/tab', 'Sandimmunneoral', 'Nexium'

               , 'Lipitor', 'Spironolactone', 'Metformin', 'Isoptin 240mg/tab'
    ...
    def plot_bbox(self, image, names, ratio, dwdh, outputs):
        for i,(batch_id,x0,y0,x1,y1,cls_id,score) in enumerate(outputs):
            ...
            name = names[cls_id]
            colors = {'Concor': [45, 73, 219], 'Isoptin 40mg/tab': [218, 115, 74], 'Sandimmunneoral': [99, 135, 62], 'Nexium': [52, 230, 38], 'Lipitor': [163, 189, 183], 'Spironolactone': [220, 125, 9], 'Metformin': [56, 17, 203], 'Isoptin 240mg/tab': [185, 155, 148], 'Cardilo': [112, 111, 84]}
    ...
    def pill_voting(self, frame_queue):
        if self.rep_pill_check == False and self.predictions.qsize() >= 9:
            ...
            DAN.push ('Pill_Detect_Result-I',  self.user_id,
                                                    self.pills['Concor'],
                                                    self.pills['Isoptin 40mg/tab'],
                                                    self.pills['Sandimmunneoral'],
                                                    self.pills['Nexium'],
                                                    self.pills['Lipitor'],
                                                    self.pills['Spironolactone'], 
                                                    self.pills['Metformin'],
                                                    self.pills['Isoptin 240mg/tab'],
                                                    self.pills['Cardilo'])
            frame = frame_queue.get()
